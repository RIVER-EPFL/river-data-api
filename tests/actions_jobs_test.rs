...
//!      cargo test --test actions_jobs_test -- --test-threads=1
...
async fn reprocess_creates_tracked_job_for_seeded_sensor() {
    ...
    let (status, text) =
        common::post_json_with_token(&app, "/api/actions/reprocess", &body, &token).await;
...
async fn refresh_aggregates_creates_tracked_job() {
    ...
    let (status, text) =
        common::post_json_with_token(&app, "/api/actions/refresh_aggregates", &body, &token).await;
...
async fn compute_derived_creates_tracked_job() {
    ...
    let (status, text) =
        common::post_json_with_token(&app, "/api/actions/compute_derived", &body, &token).await;
