...
fn write_data_perms() -> serde_json::Value {
    common::perms(true, true, false, true)
}
fn write_metadata_perms() -> serde_json::Value {
    common::perms(true, true, true, false)
}
...
async fn external_push_key_confined_to_its_project_on_every_write_path() {
    ...
    let key = common::seed_api_token(&db, write_data_perms(), Some(PROJECT_ID)).await;
    ...
    let (s, body) = common::post_json_with_token(&app, "/api/readings/batch", &batch_in, &key).await;
    ...
    let (s, _) = common::post_json_with_token(&app, "/api/readings/batch", &batch_out, &key).await;
    ...
    let (s, body) = common::post_json_with_token(&app, "/api/grab_samples", &grab_in, &key).await;
    assert_eq!(s, 200, "in-scope grab must succeed: {body}");
    let grab_out = serde_json::json!({
        "site_id": SITE_B_ID,
        "readings": [{ "parameter_id": GLOBAL_PARAM_TEMP_ID, "value": 1.5, "time": t }]
    });
    let (s, _) = common::post_json_with_token(&app, "/api/grab_samples", &grab_out, &key).await;
    ...
    let (s, _) = common::post_json_with_token(&app, "/api/status_events/batch", &status_out, &key).await;
...
async fn scoped_metadata_key_confined_and_blocked_from_global_catalog() {
    ...
    let key = common::seed_api_token(&db, write_metadata_perms(), Some(PROJECT_ID)).await;
    ...
    let (s, created) =
        common::post_json_parse_with_token(&app, "/api/site_parameters", &sp_in, &key).await;
    ...
    let (s, _) = common::post_json_with_token(&app, "/api/site_parameters", &sp_out, &key).await;
    assert_eq!(s, 403, "cross-project site_parameter create must be forbidden");

    // Resolve-by-id (delete) within the project: allowed.
    let (s, _) =
        common::delete_with_token(&app, &format!("/api/site_parameters/{new_sp_id}"), &key).await;
    ...
    let (s, _) = common::delete_with_token(&app, &format!("/api/sites/{SITE_B_ID}"), &key).await;
    ...
    let (s, _) = common::post_json_with_token(&app, "/api/parameters", &new_param, &key).await;
...
async fn scoped_key_denied_on_operator_and_global_actions() {
    ...
    let key = common::seed_api_token(&db, common::full_permissions(), Some(PROJECT_ID)).await;
    ...
    for (path, body) in cases {
        let (s, _) = common::post_json_with_token(&app, path, body, &key).await;
    ...
    let (s, _) = common::post_json_with_token(&app, "/api/readings/import_csv", &csv_out, &key).await;
...
async fn per_token_rate_limit_returns_429() {
    ...
    let limited = common::seed_api_token_with_rate_limit(&db, write_data_perms(), Some(PROJECT_ID), 2).await;
    let unlimited = common::seed_api_token(&db, write_data_perms(), Some(PROJECT_ID)).await;
    ...
    for _ in 0..8 {
        let (s, _) = common::post_json_with_token(&app, "/api/readings/batch", &body, &limited).await;
    ...
    for _ in 0..8 {
        let (s, _) = common::post_json_with_token(&app, "/api/readings/batch", &body, &unlimited).await;
...
async fn revoke_and_rotate_take_effect_immediately() {
    ...
    // --- Revoke ---
    let revoke_key = common::seed_api_token(&db, write_data_perms(), Some(PROJECT_ID)).await;
    let revoke_id = token_id(&db, &revoke_key).await;

    // Use it once so it is cached as valid.
    let (s, _) = common::get_with_token(&app, "/api/sites", &revoke_key).await;
    ...
    // --- Rotate ---
    let rotate_key = common::seed_api_token(&db, write_data_perms(), Some(PROJECT_ID)).await;
    let rotate_id = token_id(&db, &rotate_key).await;
    let (s, _) = common::get_with_token(&app, "/api/sites", &rotate_key).await;
    ...
    let (s, _) = common::get_with_token(&app, "/api/sites", &new_secret).await;
    ...
    let frozen_key = common::seed_api_token(&db, write_data_perms(), Some(PROJECT_ID)).await;
    ...
    let (s, _) = common::get_with_token(&app, "/api/sites", &new_frozen).await;
