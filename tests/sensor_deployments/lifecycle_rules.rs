...
async fn deploying_a_second_channel_keeps_the_first_open() {
    ...
    assert!(
        ...
        "temperature channel starts open"
    );
...
async fn calibration_window_absorbs_gap_to_next_valid_from() {
    ...
    assert_eq!(
        ...
        Some(40.0),
        "01-15 uses C1 (2*20), gap absorbed to C2's start"
    );
...
async fn recall_keeps_pre_first_deployment_readings_attributed() {
    ...
    assert_eq!(
        rows[0].site_id,
        Some(site1),
        "01-05 predates first deployment → keeps pairing's site_id"
    );
    assert_eq!(
        rows[1].site_id,
        Some(site1),
        "01-11 inside the (now-closed) window → still SITE1"
    );
    assert_eq!(
        rows[2].site_id, None,
        "01-15 in the post-recall gap → un-attributed"
    );
}
...
async fn deployment_recompute_only_shortens_never_extends() {
    ...
    let dep_a = e2e::create_deployment(
        &app,
        &token,
        &sid,
        SITE1_ID,
    ...
    assert!(
        wait_for_reprocessing(&db, sensor.id, WAIT).await,
        "reprocess after extend attempt"
    );
    assert_eq!(
        deployed_until(&db, &dep_a).await,
        Some(dt("2025-03-01T02:00:00Z")),
...
async fn auto_deploy_skipped_when_slot_occupied() {
    ...
    insert_unpaired_readings(
        ...
        stream,
        &[
            (dt("2025-02-01T00:00:00Z"), 10.0),
            (dt("2025-02-01T00:10:00Z"), 11.0),
        ],
    )
    .await;
    ...
    assert_eq!(
        stream_sensor, 1,
        "stream is linked to a distinct sensor B (not A)"
    );
}
...
async fn reprocess_restores_nulled_attribution_fks() {
    ...
    assert_eq!(
        rows[0].calibrated_value,
        Some(10.0),
        "calibrated_value re-derived (1*10), not 999"
    );
    assert_eq!(
        rows[0].site_id,
        Some(SITE1_ID.parse::<uuid::Uuid>().unwrap()),
        "site_id intact"
    );
}
