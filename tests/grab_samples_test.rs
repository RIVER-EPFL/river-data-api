mod common;

use sea_orm::{ConnectionTrait, Statement};
use serial_test::serial;

async fn setup() -> (axum::Router, String, sea_orm::DatabaseConnection) {
    let db = common::setup_test_db().await;
    common::cleanup_test_db(&db).await;
    common::seed_test_data(&db).await;
    let token = common::seed_api_token(&db, common::full_permissions(), None).await;
    let app = common::build_test_app(db.clone());
    (app, token, db)
}

// ============================================================================
// Basic grab sample insertion — triplicate DOC readings (~192 ppb mean)
// ============================================================================

#[tokio::test]
#[serial]
async fn test_insert_triplicate_grab_samples() {
    let (app, token, db) = setup().await;

    let time = "2025-06-15T10:00:00Z";
    let (status, body) = common::post_json_with_token(
        &app,
        "/api/service/grab_samples",
        &serde_json::json!({
            "site_id": common::SITE1_ID,
            "created_by": "test-user",
            "readings": [
                { "parameter_id": common::GLOBAL_PARAM_TEMP_ID, "value": 185.2, "time": time },
                { "parameter_id": common::GLOBAL_PARAM_TEMP_ID, "value": 198.7, "time": time },
                { "parameter_id": common::GLOBAL_PARAM_TEMP_ID, "value": 191.4, "time": time }
            ]
        }),
        &token,
    )
    .await;
    assert_eq!(status, 200, "insert should succeed: {body}");
    let json: serde_json::Value = serde_json::from_str(&body).unwrap();
    assert_eq!(json["inserted"], 3);
    assert_eq!(json["samples_created"], 1, "3 replicates should create 1 sample");

    // Verify readings were inserted with correct site_id and parameter_id
    let row = db
        .query_one(Statement::from_string(
            sea_orm::DatabaseBackend::Postgres,
            format!(
                "SELECT COUNT(*) as c FROM readings \
                 WHERE site_id = '{}' AND parameter_id = '{}' \
                 AND time = '{time}'",
                common::SITE1_ID,
                common::GLOBAL_PARAM_TEMP_ID
            ),
        ))
        .await
        .unwrap()
        .unwrap();
    let count: i64 = row.try_get("", "c").unwrap();
    assert_eq!(count, 3, "should have 3 readings for this time+parameter");

    // Verify sample row was created
    let row = db
        .query_one(Statement::from_string(
            sea_orm::DatabaseBackend::Postgres,
            format!(
                "SELECT COUNT(*) as c FROM samples \
                 WHERE site_id = '{}' AND parameter_id = '{}'",
                common::SITE1_ID,
                common::GLOBAL_PARAM_TEMP_ID
            ),
        ))
        .await
        .unwrap()
        .unwrap();
    let sample_count: i64 = row.try_get("", "c").unwrap();
    assert_eq!(sample_count, 1, "should have 1 sample record");
}

// ============================================================================
// Single reading (no replicates) — should NOT create a sample
// ============================================================================

#[tokio::test]
#[serial]
async fn test_single_grab_sample_no_sample_row() {
    let (app, token, db) = setup().await;

    let (status, body) = common::post_json_with_token(
        &app,
        "/api/service/grab_samples",
        &serde_json::json!({
            "site_id": common::SITE1_ID,
            "readings": [
                { "parameter_id": common::GLOBAL_PARAM_DO_ID, "value": 330.0, "time": "2025-06-15T11:00:00Z" }
            ]
        }),
        &token,
    )
    .await;
    assert_eq!(status, 200, "insert should succeed: {body}");
    let json: serde_json::Value = serde_json::from_str(&body).unwrap();
    assert_eq!(json["inserted"], 1);
    assert_eq!(json["samples_created"], 0, "single reading should not create a sample");

    // Verify no sample row for this parameter+time
    let row = db
        .query_one(Statement::from_string(
            sea_orm::DatabaseBackend::Postgres,
            format!(
                "SELECT COUNT(*) as c FROM samples \
                 WHERE site_id = '{}' AND parameter_id = '{}' \
                 AND collected_at = '2025-06-15T11:00:00Z'",
                common::SITE1_ID,
                common::GLOBAL_PARAM_DO_ID
            ),
        ))
        .await
        .unwrap()
        .unwrap();
    let count: i64 = row.try_get("", "c").unwrap();
    assert_eq!(count, 0);
}

// ============================================================================
// Multiple parameters in one request — mixed replicates
// ============================================================================

#[tokio::test]
#[serial]
async fn test_multi_parameter_grab_samples() {
    let (app, token, _db) = setup().await;

    let time = "2025-06-15T12:00:00Z";
    let (status, body) = common::post_json_with_token(
        &app,
        "/api/service/grab_samples",
        &serde_json::json!({
            "site_id": common::SITE1_ID,
            "readings": [
                { "parameter_id": common::GLOBAL_PARAM_TEMP_ID, "value": 7.5, "time": time },
                { "parameter_id": common::GLOBAL_PARAM_TEMP_ID, "value": 7.8, "time": time },
                { "parameter_id": common::GLOBAL_PARAM_COND_ID, "value": 432.0, "time": time }
            ]
        }),
        &token,
    )
    .await;
    assert_eq!(status, 200, "insert should succeed: {body}");
    let json: serde_json::Value = serde_json::from_str(&body).unwrap();
    assert_eq!(json["inserted"], 3);
    assert_eq!(json["samples_created"], 1, "only Temp has 2+ replicates");
}

// ============================================================================
// Auto-creates grab_sample stream
// ============================================================================

#[tokio::test]
#[serial]
async fn test_grab_sample_creates_stream() {
    let (app, token, db) = setup().await;

    let (status, _) = common::post_json_with_token(
        &app,
        "/api/service/grab_samples",
        &serde_json::json!({
            "site_id": common::SITE1_ID,
            "readings": [
                { "parameter_id": common::GLOBAL_PARAM_TURB_ID, "value": 25.0, "time": "2025-06-15T13:00:00Z" }
            ]
        }),
        &token,
    )
    .await;
    assert_eq!(status, 200);

    // Verify a grab_sample stream was created
    let row = db
        .query_one(Statement::from_string(
            sea_orm::DatabaseBackend::Postgres,
            format!(
                "SELECT COUNT(*) as c FROM data_streams \
                 WHERE source_system = 'grab_sample' \
                 AND source_key = '{}:{}'",
                common::SITE1_ID,
                common::GLOBAL_PARAM_TURB_ID
            ),
        ))
        .await
        .unwrap()
        .unwrap();
    let count: i64 = row.try_get("", "c").unwrap();
    assert_eq!(count, 1, "should have created a grab_sample stream");
}

// ============================================================================
// Invalid site returns 404
// ============================================================================

#[tokio::test]
#[serial]
async fn test_grab_sample_invalid_site() {
    let (app, token, _db) = setup().await;

    let (status, _) = common::post_json_with_token(
        &app,
        "/api/service/grab_samples",
        &serde_json::json!({
            "site_id": "00000000-0000-4000-a000-999999999999",
            "readings": [
                { "parameter_id": common::GLOBAL_PARAM_TEMP_ID, "value": 10.0, "time": "2025-06-15T14:00:00Z" }
            ]
        }),
        &token,
    )
    .await;
    assert_eq!(status, 404);
}

// ============================================================================
// Invalid parameter for site returns 400
// ============================================================================

#[tokio::test]
#[serial]
async fn test_grab_sample_invalid_parameter() {
    let (app, token, _db) = setup().await;

    // Site2 doesn't have DEPTH parameter
    let (status, _) = common::post_json_with_token(
        &app,
        "/api/service/grab_samples",
        &serde_json::json!({
            "site_id": common::SITE2_ID,
            "readings": [
                { "parameter_id": common::GLOBAL_PARAM_DEPTH_ID, "value": 300.0, "time": "2025-06-15T15:00:00Z" }
            ]
        }),
        &token,
    )
    .await;
    assert_eq!(status, 400, "DEPTH is not configured for Site2");
}

// ============================================================================
// Empty readings returns 400
// ============================================================================

#[tokio::test]
#[serial]
async fn test_grab_sample_empty_readings() {
    let (app, token, _db) = setup().await;

    let (status, _) = common::post_json_with_token(
        &app,
        "/api/service/grab_samples",
        &serde_json::json!({
            "site_id": common::SITE1_ID,
            "readings": []
        }),
        &token,
    )
    .await;
    assert_eq!(status, 400);
}

// ============================================================================
// Permission check — write_data required
// ============================================================================

#[tokio::test]
#[serial]
async fn test_grab_samples_require_write_data() {
    let db = common::setup_test_db().await;
    common::cleanup_test_db(&db).await;
    common::seed_test_data(&db).await;
    let token = common::seed_api_token(
        &db,
        serde_json::json!({ "read_data": true, "read_metadata": true }),
        None,
    )
    .await;
    let app = common::build_test_app(db);

    let (status, _) = common::post_json_with_token(
        &app,
        "/api/service/grab_samples",
        &serde_json::json!({
            "site_id": common::SITE1_ID,
            "readings": [
                { "parameter_id": common::GLOBAL_PARAM_TEMP_ID, "value": 10.0, "time": "2025-06-15T16:00:00Z" }
            ]
        }),
        &token,
    )
    .await;
    assert_eq!(status, 403, "grab_samples should require write_data permission");
}
