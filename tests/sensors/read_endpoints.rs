...
async fn sensor_read_substrate_endpoints() {
    ...
    assert_eq!(
        body["point_count"].as_i64().unwrap(),
        6,
        "all six in the window"
    );
    ...
    let param_bands = body["bands"][crate::common::GLOBAL_PARAM_TEMP_ID]
        .as_array()
        .unwrap();
    assert_eq!(
        param_bands.len(),
        1,
        "one deployment band for the parameter"
    );
    assert_eq!(
        param_bands[0]["sensor_id"],
        serde_json::json!(sensor.id.to_string())
    );
    ...
    assert!(
        param_cals
            .iter()
            .any(|c| c["calibration_id"] == serde_json::json!(cal.to_string())),
