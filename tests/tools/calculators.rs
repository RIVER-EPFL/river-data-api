...
async fn test_list_tools_excludes_removed() {
    ...
    for expected in [
        "doc", "tss_afdm", "chlorophyll", "nutrients", "alkalinity", "pco2",
        "dic", "dom", "field_data", "co2_air", "benthic", "chla_benthic",
    ] {
...
async fn test_doc_with_std_curve() {
    ...
    let json = calculate(&app, "doc", serde_json::json!({
...
async fn test_tss_afdm() {
    ...
    let json = calculate(&app, "tss_afdm", serde_json::json!({
        "wgt_dried_g": 0.1005,
        "wgt_prefilt_g": 0.1,
...
async fn test_chlorophyll_acid_and_noacid() {
    ...
    let json = calculate(&app, "chlorophyll", serde_json::json!({
        "method": "acid",
...
async fn test_dic_replicates_with_lab_temp() {
    ...
    let json = calculate(&app, "dic", serde_json::json!({
...
async fn test_pco2_simple_mode() {
    ...
    let json = calculate(&app, "pco2", serde_json::json!({
        "co2_aq_umol": 50.0,
        "water_temp_c": 25.0
    }), &token).await;
...
async fn test_co2_air_ch4_dry_only() {
    ...
    let json = calculate(&app, "co2_air", serde_json::json!({
        "ch4_wet": 2000.0,
        "h2o_percent": 1.5
    }), &token).await;
...
async fn test_dom_suva_and_ratios() {
    ...
    let json = calculate(&app, "dom", serde_json::json!({
        "a254": 0.15,
        "doc_avg_ppb": 2500.0,
        "peak_a": 0.4,
        "peak_c": 0.2,
...
async fn test_field_data_baro_and_co2_correction() {
    ...
    let json = calculate(&app, "field_data", serde_json::json!({
        "elevation_m": 470.0,
        "temp_c": 15.0
    }), &token).await;
...
async fn test_benthic() {
    ...
    let json = calculate(&app, "benthic", serde_json::json!({
...
async fn test_chla_benthic_full_chain() {
    ...
    let json = calculate(&app, "chla_benthic", serde_json::json!({
...
async fn test_alkalinity_fills_ph_when_missing() {
    ...
    let json = calculate(&app, "alkalinity", serde_json::json!({
        "Alk_meqL": 2.5,
        "Alk_mgL": 125.1,
        "Alk_w_weight_g": 50.0,
        "Alk_init_pH": 8.1
