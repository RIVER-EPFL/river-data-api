...
async fn an_output_resolves_by_id_then_by_code() {
    if !crate::common::profile::Service::ToolsRunner
        .require("an_output_resolves_by_id_then_by_code")
        .await
    {
        return;
    }
    if !crate::common::profile::Service::Keycloak
        .require("tool_output_resolution")
        .await
    {
        return;
    }
...
async fn an_unknown_id_is_refused_and_an_unknown_code_is_reported() {
    if !crate::common::profile::Service::ToolsRunner
        .require("an_unknown_id_is_refused_and_an_unknown_code_is_reported")
        .await
    {
        return;
    }
    if !crate::common::profile::Service::Keycloak
        .require("tool_output_catalog_findings")
        .await
    {
        return;
    }
...
async fn an_id_and_a_code_naming_different_parameters_are_refused() {
    if !crate::common::profile::Service::ToolsRunner
        .require("an_id_and_a_code_naming_different_parameters_are_refused")
        .await
    {
        return;
    }
    if !crate::common::profile::Service::Keycloak
        .require("tool_output_disagreement")
        .await
    {
        return;
    }
...
async fn two_outputs_resolving_to_one_parameter_are_refused() {
    if !crate::common::profile::Service::ToolsRunner
        .require("two_outputs_resolving_to_one_parameter_are_refused")
        .await
    {
        return;
    }
    if !crate::common::profile::Service::Keycloak
        .require("tool_output_collision")
        .await
    {
        return;
    }
...
async fn a_dangling_parameter_id_is_flagged_and_reported_at_activation() {
    if !crate::common::profile::Service::ToolsRunner
        .require("a_dangling_parameter_id_is_flagged_and_reported_at_activation")
        .await
    {
        return;
    }
    if !crate::common::profile::Service::Keycloak
        .require("tool_output_dangling_id")
        .await
    {
        return;
    }
...
async fn a_malformed_parameter_id_names_the_output_it_sits_on() {
    if !crate::common::profile::Service::ToolsRunner
        .require("a_malformed_parameter_id_names_the_output_it_sits_on")
        .await
    {
        return;
    }
    if !crate::common::profile::Service::Keycloak
        .require("tool_output_malformed_id")
        .await
    {
        return;
    }
...
async fn a_manifest_declaring_a_nonexistent_constant_is_refused() {
    if !crate::common::profile::Service::ToolsRunner
        .require("a_manifest_declaring_a_nonexistent_constant_is_refused")
        .await
    {
        return;
    }
    if !crate::common::profile::Service::Keycloak
        .require("tool_constant_findings")
        .await
    {
        return;
    }
