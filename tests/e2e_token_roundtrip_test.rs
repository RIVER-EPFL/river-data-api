...
async fn write_only_key_cannot_read_back_separate_read_key_can() {
    ...
    let writer = common::seed_api_token(&db, common::perms(false, false, false, true), None).await;
    let reader = common::seed_api_token(&db, common::perms(false, true, false, false), None).await;
    ...
    let (s, body) = common::post_json_with_token(&app, "/api/readings/batch", &batch, &writer).await;
    ...
    let (s, _) = common::get_with_token(&app, &readings_url, &writer).await;
    ...
    let (s, body) = common::get_json_with_token(&app, &readings_url, &reader).await;
    ...
    let (s, _) = common::post_json_with_token(&app, "/api/readings/batch", &batch, &reader).await;
