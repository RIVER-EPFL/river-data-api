...
async fn test_simple_reassign() {
    ...
    let result = merge_parameters(&db, &merge_req(common::GLOBAL_PARAM_DO_ID, common::GLOBAL_PARAM_TEMP_ID))
        .await
...
async fn test_cross_site() {
    ...
    let result = merge_parameters(&db, &merge_req(common::GLOBAL_PARAM_DEPTH_ID, common::GLOBAL_PARAM_TURB_ID))
        .await
        .expect("merge should succeed");
...
async fn test_aliases_absorbed() {
    ...
    let target_aliases = aliases(&db, common::GLOBAL_PARAM_TEMP_ID).await;
    assert!(
...
async fn test_same_id_rejection() {
    ...
    let result = merge_parameters(
        &db,
        &merge_req(common::GLOBAL_PARAM_TEMP_ID, common::GLOBAL_PARAM_TEMP_ID),
    )
    .await;
...
async fn test_not_found_rejection() {
    ...
    let result = merge_parameters(
        &db,
        &merge_req("00000000-0000-4000-b000-999999999999", common::GLOBAL_PARAM_TEMP_ID),
    )
    .await;
...
async fn test_http_round_trip() {
    ...
    let body = serde_json::json!({
        "source_parameter_id": common::GLOBAL_PARAM_DO_ID,
        "target_parameter_id": common::GLOBAL_PARAM_TEMP_ID,
    });
    ...
    let (get_status, _) = common::get_with_token(
        &app,
        &format!("/api/parameters/{}", common::GLOBAL_PARAM_DO_ID),
        &token,
    )
    .await;
