...
//!
//! Run: cargo test --test alarm_consistency_test -- --test-threads=1
...
async fn every_path_agrees_on_a_parameter_default_breach() {
    ...
    let site = common::SITE1_ID;
    let turb = common::GLOBAL_PARAM_TURB_ID;

    // Only a parameter default — no threshold rows anywhere.
    common::exec(&db, "DELETE FROM alarm_thresholds").await;
    common::exec(&db, "DELETE FROM alarm_events").await;
    common::exec(
        &db,
    ...
    let (s, b) = common::get_json_with_token(
        &app,
    ...
    let (s, b) = common::get_json_with_token(&app, "/api/alarms/active", &token).await;
    ...
    let (s, b) =
        common::get_json_with_token(&app, &format!("/api/sites/{site}/alarms?{win}"), &token).await;
    ...
    common::refresh_continuous_aggregates(&db).await;
    let (s, b) = common::get_json_with_token(
        &app,
