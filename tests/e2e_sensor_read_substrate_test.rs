...
//!
//! Run: cargo test --test e2e_sensor_read_substrate_test -- --test-threads=1
...
async fn sensor_read_substrate_endpoints() {
    ...
    let sensor = sl::create_sensor(&db, "substrate", common::GLOBAL_PARAM_TEMP_ID).await;
    ...
    let stream = sl::create_paired_stream(&db, "substrate", common::PARAM_S1_TEMP_ID).await;
    let raw: Vec<(_, f64)> = (0..6)
        .map(|i| (sl::dt(&format!("2025-06-01T00:{:02}:00Z", i * 10)), 10.0 + i as f64))
        .collect();
    sl::insert_readings(
        &db, stream, common::SITE1_ID, common::GLOBAL_PARAM_TEMP_ID, sensor.id, cal, dep, 2.0, 1.0, &raw,
    )
    .await;
    ...
    assert_eq!(bands[0]["site_id"], serde_json::json!(common::SITE1_ID));
    ...
    let param_bands = body["bands"][common::GLOBAL_PARAM_TEMP_ID].as_array().unwrap();
    ...
    let param_cals = body["calibrations"][common::GLOBAL_PARAM_TEMP_ID].as_array().unwrap();
