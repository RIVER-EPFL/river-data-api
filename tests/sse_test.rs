...
async fn setup() -> (
    ...
    let token = common::seed_token_read_data_only(&db).await;
    let (app, events) = common::build_test_app_with_events(db.clone());
    (app, token, events, db)
}
...
async fn sse_requires_read_data_permission() {
    ...
    let token = common::seed_token_read_metadata_only(&db).await;
