...
//!
//! Run: cargo test --test e2e_aggregation_and_scatter_test -- --test-threads=1
...
async fn aggregation_resolutions_and_paired_parameter_series() {
    ...
    for res in ["hourly", "daily", "weekly", "monthly"] {
        ...
        let (status, agg) = common::get_json_with_token(&app, &uri, &token).await;
        assert_eq!(status, 200, "{res} aggregate ({status}): {agg}");
        let avg = e2e::field_for(&agg, common::GLOBAL_PARAM_DO_ID, "avg");
    ...
    let uri = format!(
        ...
        common::GLOBAL_PARAM_DO_ID, common::GLOBAL_PARAM_COND_ID
    );
    ...
    let do_vals = e2e::values_for(&readings, common::GLOBAL_PARAM_DO_ID);
    let cond_vals = e2e::values_for(&readings, common::GLOBAL_PARAM_COND_ID);
