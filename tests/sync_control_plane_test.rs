...
//!
//! Run: cargo test --test sync_control_plane_test -- --test-threads=1
...
async fn enroll_with_seeded_credentials_returns_session_token() {
    ...
    common::seed_sync_credentials(&db, "svc_test_client", "super-secret", "test").await;
...
async fn enroll_with_bad_credentials_is_rejected() {
    let db = common::setup_test_db().await;
    common::cleanup_test_db(&db).await;
    common::seed_sync_credentials(&db, "svc_x", "right-secret", "test").await;
    let app = common::build_test_app(db.clone());
...
async fn heartbeat_updates_service_and_returns_no_pending_commands() {
    let db = common::setup_test_db().await;
    common::cleanup_test_db(&db).await;
    let (token, service_id) = common::seed_sync_session_token(&db).await;
    let app = common::build_test_app(db.clone());
...
async fn command_lifecycle_issue_deliver_acknowledge_complete() {
    ...
    common::cleanup_test_db(&db).await;
    let (token, service_id) = common::seed_sync_session_token(&db).await;
    let admin = common::seed_token_full(&db).await;
    let app = common::build_test_app(db.clone());

    let (status, cmd) = common::post_json_parse_with_token(
        &app,
    ...
    let (status, hb) = common::post_json_parse_with_token(
        &app,
        "/api/sync/heartbeat",
    ...
    let (status, _) = common::patch_json_with_token(
        &app,
    ...
    let (other_token, _other_service) = common::seed_sync_session_token(&db).await;
    let (status, _) = common::patch_json_with_token(
        &app,
        &format!("/api/sync/commands/{command_id}"),
...
async fn sync_event_create_update_and_read_back() {
    ...
    let (status, ev) = common::post_json_parse_with_token(
        &app,
        "/api/sync/events",
        &serde_json::json!({
    ...
    let (status, events) = common::get_json_with_token(&app, "/api/sync/events", &admin).await;
...
async fn health_state_derived_from_heartbeat_recency() {
    let db = common::setup_test_db().await;
    common::cleanup_test_db(&db).await;
    let read_token = common::seed_token_read_metadata_only(&db).await;
    ...
    let app = common::build_test_app(db.clone());
    let (status, services) = common::get_json_with_token(&app, "/api/sync/services", &read_token).await;
    ...
    let (status, one) =
        common::get_json_with_token(&app, &format!("/api/sync/services/{healthy}"), &read_token).await;
