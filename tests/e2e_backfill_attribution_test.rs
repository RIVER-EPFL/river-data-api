...
//!
//! Run: cargo test --test e2e_backfill_attribution_test -- --test-threads=1
...
async fn count_slot_orphans(db: &sea_orm::DatabaseConnection) -> i64 {
    let row = db
        .query_one(Statement::from_sql_and_values(
            ...
            [common::SITE1_ID.into(), common::GLOBAL_PARAM_TEMP_ID.into()],
        ))
        .await
        .unwrap()
        .unwrap();
...
async fn backfill_attributes_pre_deployment_history() {
    ...
    let sensor = sl::create_sensor(&db, "backfill-probe", common::GLOBAL_PARAM_TEMP_ID).await;
    let t0 = sl::dt("2025-06-01T00:00:00Z");
    let dep = sl::deploy_sensor(&db, sensor.id, common::SITE1_ID, t0).await;
    let stream = sl::create_paired_stream(&db, "backfill", common::PARAM_S1_TEMP_ID).await;
    sl::insert_readings(
        &db, stream, common::SITE1_ID, common::GLOBAL_PARAM_TEMP_ID,
    ...
    let (status, body) =
        common::get_json_with_token(&app, "/api/actions/backfill_candidates", &token).await;
...
async fn backfill_is_bounded_by_a_prior_deployment() {
    ...
    let a = sl::create_sensor(&db, "prior-A", common::GLOBAL_PARAM_TEMP_ID).await;
    let dep_a = sl::deploy_sensor(&db, a.id, common::SITE1_ID, sl::dt("2025-01-01T00:00:00Z")).await;
    sl::end_deployment(&db, dep_a, sl::dt("2025-04-15T00:00:00Z")).await;
    let b = sl::create_sensor(&db, "current-B", common::GLOBAL_PARAM_TEMP_ID).await;
    let dep_b = sl::deploy_sensor(&db, b.id, common::SITE1_ID, sl::dt("2025-06-01T00:00:00Z")).await;

    // Orphan inside A's window + orphan in the gap between A and B.
    let stream = sl::create_paired_stream(&db, "bounded", common::PARAM_S1_TEMP_ID).await;
    sl::insert_orphan_readings(
        &db, stream, common::SITE1_ID, common::GLOBAL_PARAM_TEMP_ID,
        &[
