...
//!
//! Run: cargo test --test e2e_sensor_lifecycle_test -- --test-threads=1
...
async fn deploy_move_recall_reprocess_over_http() {
    ...
    let site2 = Uuid::parse_str(common::SITE2_ID).unwrap();
    ...
    let sensor = sl::create_sensor(&db, "lifecycle", common::GLOBAL_PARAM_TEMP_ID).await;
    ...
    let stream = sl::create_paired_stream(&db, "lifecycle", common::PARAM_S1_TEMP_ID).await;
    ...
    let _dep_b = e2e::create_deployment(&app, &token, &sensor_id, common::SITE2_ID, "2025-06-01T00:30:00Z").await;
    assert!(
    ...
    let (status, repro) = common::post_json_with_token(
        &app,
