...
//!
//! Run: cargo test --test import_adopt_test -- --test-threads=1
...
async fn import_then_adopt_backfills_by_window() {
    ...
    let temp = Uuid::parse_str(common::GLOBAL_PARAM_TEMP_ID).unwrap();
    ...
    let (status, body) = common::post_json_with_token(
        ...
        &serde_json::json!({ "parameter_id": common::GLOBAL_PARAM_TEMP_ID }),
        &token,
    )
    .await;
    ...
    let (_s, body2) = common::post_json_with_token(
        &app,
        &format!("/api/streams/{stream}/import"),
        &serde_json::json!({ "parameter_id": common::GLOBAL_PARAM_TEMP_ID }),
        &token,
    ...
    let (status, body) = common::post_json_with_token(
        ...
        &serde_json::json!({ "site_id": common::SITE1_ID, "deployed_from": "2025-05-01T00:00:00Z" }),
        &token,
    )
    .await;
    ...
    assert_eq!(
        common::e2e::poll_job(&app, &token, job_id, 30).await,
        "completed",
...
async fn adopt_rejects_occupied_slot() {
    ...
    let sensor_a = sl::create_sensor(&db, "occ-a", common::GLOBAL_PARAM_TEMP_ID).await;
    let sensor_b = sl::create_sensor(&db, "occ-b", common::GLOBAL_PARAM_TEMP_ID).await;

    let (status, body) = common::post_json_with_token(
        &app,
        &format!("/api/sensors/{}/adopt", sensor_a.id),
        &serde_json::json!({ "site_id": common::SITE1_ID, "deployed_from": "2025-06-01T00:00:00Z" }),
        &token,
    )
    .await;
    ...
    let (status, body) = common::post_json_with_token(
        ...
        &serde_json::json!({ "site_id": common::SITE1_ID, "deployed_from": "2025-06-02T00:00:00Z" }),
        &token,
    )
    .await;
...
async fn swap_ends_a_starts_b() {
    ...
    let sensor_a = sl::create_sensor(&db, "swap-a", common::GLOBAL_PARAM_TEMP_ID).await;
    let sensor_b = sl::create_sensor(&db, "swap-b", common::GLOBAL_PARAM_TEMP_ID).await;
    ...
    assert!(
        common::e2e::poll_job(&app, &token, swap["incoming_job_id"].as_str().unwrap(), 30).await
            == "completed"
    );
    ...
    let open_count: i64 = db
        .query_one(Statement::from_sql_and_values(
            ...
            [
                Uuid::parse_str(common::SITE1_ID).unwrap().into(),
                Uuid::parse_str(common::GLOBAL_PARAM_TEMP_ID).unwrap().into(),
            ],
        ))
        .await
        .unwrap()
