...
impl Fixture {
    ...
    fn codes(&self) -> (String, String) {
        ...
        (
            self.track.parameters[0].0.clone(),
            self.track.parameters[1].0.clone(),
        )
    }
}
...
async fn dry_run_stages_session_and_import_reuses_it_without_csv() {
    ...
    let do_id =
        crate::common::e2e::create_parameter(&fx.app, &fx.admin, "DOuM", "Dissolved Oxygen", "uM")
            .await;
    let temp_id = crate::common::e2e::create_parameter(
        &fx.app,
        &fx.admin,
        "WaterTempDegC",
        "Water Temperature",
    ...
    for pid in [&do_id, &temp_id] {
        crate::common::e2e::assign_site_parameter_minimal(
            &fx.app,
            &fx.admin,
            &fx.track.site_id,
            pid,
    ...
    assert_eq!(
        status, 200,
        "a river-level member previews an import: {preview}"
    );
    assert_eq!(
        preview["dry_run"], true,
        "the preview reports itself as one: {preview}"
    );
    assert_eq!(
        preview["row_count"], VIEWLINC_ROWS,
        "every data row parses: {preview}"
    );
    assert_eq!(
        preview["inserted_total"], 0,
        "a preview writes nothing: {preview}"
    );
    assert_eq!(
    ...
    assert_eq!(
        status, 200,
        "the staged session is re-previewable without the CSV: {remapped}"
    );
    assert_eq!(
        session_of(&remapped),
        session,
    ...
    assert_eq!(
        status, 200,
        "the import runs off the staged session: {imported}"
    );
    assert_eq!(imported["dry_run"], false, "{imported}");
    assert_eq!(
        session_of(&imported),
        session,
        "the same session serves the write: {imported}"
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        "nothing was already stored: {imported}"
    );
    assert_eq!(
    ...
    assert_eq!(
        count(
            &fx.db,
            &format!("SELECT count(*) AS c FROM readings WHERE site_id = '{site}'")
        )
        .await,
    ...
    assert_eq!(
        count(
            &fx.db,
            &format!(
                "SELECT count(DISTINCT parameter_id) AS c FROM readings WHERE site_id = '{site}'"
            )
        )
        .await,
        2,
    ...
    assert_eq!(
        staging_rows(&fx.db).await,
        0,
        "the worker drops its staged rows on completion"
    );
    ...
    assert_eq!(
        status, 200,
        "the member who ran the import can watch its job: {job}"
    );
}
...
async fn unresolvable_session_is_rejected_by_message_and_full_csv_retry_succeeds() {
    ...
    let (status, preview) = import(
        &fx,
        &json!({ "site": fx.track.site_id, "csv": csv, "dry_run": true }),
    )
    .await;
    ...
    assert_eq!(
        status, 400,
        "a request with neither CSV nor session is refused: {neither}"
    );
    assert!(
    ...
    let (status, retried) = import(
        &fx,
        &json!({ "site": fx.track.site_id, "csv": csv, "conflict": "skip" }),
    )
    .await;
    ...
    assert_eq!(
        retried["inserted_total"], 4,
        "two rows across two columns: {retried}"
    );
...
async fn dry_run_previews_overlap_diff_then_overwrite_applies_exactly_those_rows() {
    ...
    assert_eq!(
        first["overlaps_identical"], 0,
        "an empty slot has no overlap: {first}"
    );
    assert_eq!(first["overlaps_differing"], 0, "{first}");
    let job_id = first["derived_job_id"]
        .as_str()
        .expect("first import enqueues a job")
        .to_string();
    assert_eq!(
        crate::common::e2e::poll_job(&fx.app, &fx.admin, &job_id, 60).await,
        "completed",
    ...
    assert_eq!(
        diff["overlaps_identical"], 3,
        "three cells are unchanged: {diff}"
    );
    assert_eq!(diff["overlaps_differing"], 1, "one cell changed: {diff}");
    assert_eq!(
        diff["inserted_total"], 0,
        "a preview inserts nothing: {diff}"
    );
    ...
    assert_eq!(
        sample.len(),
        1,
        "the preview lists the one differing row: {diff}"
    );
    assert_eq!(sample[0]["existing"], 150.0, "what is stored today: {diff}");
    assert_eq!(
        sample[0]["incoming"], 155.0,
        "what the file would write: {diff}"
    );
    assert_eq!(
        sample[0]["parameter_id"], depth_id,
        "attributed to the right parameter: {diff}"
    );
    assert_eq!(
    ...
    assert_eq!(
        status, 200,
        "confirming the overwrite ({status}): {applied}"
    );
    assert_eq!(applied["inserted_total"], 0, "nothing is new: {applied}");
    assert_eq!(
        applied["duplicates"], 3,
        "the identical cells are left alone: {applied}"
    );
    assert_eq!(
        applied["overwritten"], 1,
        "exactly the previewed row is replaced: {applied}"
    );
    let job_id = applied["derived_job_id"]
        .as_str()
        .unwrap_or_else(|| {
            panic!("an all-overlap file still has work in overwrite mode: {applied}")
        })
        .to_string();
    assert_eq!(
        crate::common::e2e::poll_job(&fx.app, &fx.admin, &job_id, 60).await,
...
async fn conflict_skip_reports_the_differing_row_and_leaves_it_stored() {
    ...
    assert_eq!(
        ...
        "every incoming cell already exists: {skipped}"
    );
    assert_eq!(
    ...
    assert_eq!(
        skipped["overwritten"], 0,
        "skip mode replaces nothing: {skipped}"
    );
    assert!(
    ...
    assert_eq!(
        staging_rows(&fx.db).await,
        0,
        "a no-work import stages nothing"
    );
}
...
async fn tz_offset_hours_converts_to_utc_and_is_not_baked_into_the_session() {
    ...
    let (status, corrected) = import(
        &fx,
        &json!({ "site": site, "session_id": session, "dry_run": true }),
    )
    .await;
    assert_eq!(status, 200, "preview at UTC ({status}): {corrected}");
    assert_eq!(
        session_of(&corrected),
        session,
        "still the same session: {corrected}"
    );
    assert_eq!(
