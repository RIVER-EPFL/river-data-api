...
async fn test_unauthenticated_get_returns_401() {
    ...
    let (status, _body) = common::get(&app, "/api/projects").await;
...
async fn test_expired_token_returns_401() {
    ...
    let expired_token = common::seed_api_token_with_expiry(
        &db,
    ...
    let (status, _body) =
        common::get_with_token(&app, "/api/projects", &expired_token).await;
...
async fn test_inactive_token_returns_401() {
    ...
    let (status, _body) =
        common::get_with_token(&app, "/api/projects", &inactive_token).await;
...
async fn test_read_metadata_only_token() {
    ...
    let (status, _body) =
        common::get_with_token(&app, "/api/projects", &token).await;
...
async fn test_malformed_auth_header_returns_401() {
    ...
    let (status, _body) =
        common::get_with_auth_header(&app, "/api/projects", "NotBearer xyz").await;
...
async fn test_empty_bearer_token_returns_401() {
    ...
    let (status, _body) =
        common::get_with_auth_header(&app, "/api/projects", "Bearer ").await;
