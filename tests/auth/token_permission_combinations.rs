...
async fn all_sixteen_permission_combinations_enforced() {
    ...
    for bits in 0u8..16 {
        ...
        .await;
        let label =
            format!("rm={read_metadata} rd={read_data} wm={write_metadata} wd={write_data}");
        ...
        assert!(
            read_metadata || s == 403,
            "[{label}] GET /api/sites should 403 without read_metadata, got {s}"
        );
        ...
        assert_eq!(
            s == 200,
            read_data,
            "[{label}] GET /api/annotations needs read_data: got {s}"
        );
        let (s, _) = crate::common::get_with_token(&app, "/api/samples", &token).await;
        assert_eq!(
            s == 200,
            read_data,
            "[{label}] GET /api/samples needs read_data: got {s}"
        );
        ...
        let (s, _) =
            crate::common::post_json_with_token(&app, "/api/parameters", &new_param, &token).await;
        if write_metadata {
            assert_ne!(
                s, 403,
                "[{label}] write_metadata key must not be 403 on parameter create"
            );
        } else {
            assert_eq!(
                s, 403,
                "[{label}] non-write_metadata key must be 403 on parameter create, got {s}"
            );
        }
        ...
        let (s, _) =
            crate::common::post_json_with_token(&app, "/api/readings/batch", &batch, &token).await;
        if write_data {
            assert_ne!(
                s, 403,
                "[{label}] write_data key must not be 403 on batch ingest"
            );
        } else {
            assert_eq!(
                s, 403,
                "[{label}] non-write_data key must be 403 on batch ingest, got {s}"
            );
        }
    }
}
