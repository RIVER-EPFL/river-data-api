...
//!      cargo test --test reprocessing_test -- --test-threads=1
...
async fn recalibration_updates_all_readings() {
    ...
    let (status, json) = get_json_with_token(
        &app,
        &format!(
            "/api/service/sites/{}/readings?start=2025-01-01T00:00:00Z&end=2025-01-02T00:00:00Z\
...
async fn deployment_change_updates_site_and_deployment() {
    ...
    let (status, _) = put_json_with_token(
        &app,
        &format!("/api/service/sensor_deployments/{dep_a}"),
    ...
    let (status, body) = post_json_with_token(
        &app,
        "/api/service/sensor_deployments",
        &serde_json::json!({
            "sensor_id": sensor.id,
            "site_id": SITE2_ID,
...
async fn retroactive_calibration_date_change() {
    ...
    let (status, _) = put_json_with_token(
        &app,
        &format!("/api/service/sensor_calibrations/{real_cal}"),
...
async fn delete_intermediate_calibration_fallback() {
    ...
    let (status, _) = delete_with_token(
        &app,
        &format!("/api/service/sensor_calibrations/{cal_a}"),
        &token,
    )
    .await;
