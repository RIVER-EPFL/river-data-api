...
async fn configure_threshold_annotate_and_note() {
    ...
    let (status, thr) = crate::common::post_json_parse_with_token(
        &app,
    ...
    let (status, note) = crate::common::post_json_parse_with_token(
        &app,
        "/api/notes",
...
async fn alarm_acknowledge_and_autoresolve() {
    let db = crate::common::setup_test_db().await;
    crate::common::cleanup_test_db(&db).await;
    crate::common::seed_test_data(&db).await; // thresholds + in-range readings for SITE1/Turbidity (OK band 0..100)
    let token = crate::common::seed_api_token(&db, crate::common::full_permissions(), None).await;
    let app = crate::common::build_test_app(db.clone());
