...
async fn backfill_attributes_pre_deployment_history() {
    ...
    assert_eq!(
        &cands[0]["target_from"].as_str().unwrap()[..10],
        "2025-03-01"
    );
    ...
    let (_s, body2) =
        crate::common::get_json_with_token(&app, "/api/actions/backfill_candidates", &token).await;
    assert_eq!(
        body2["total_candidates"].as_i64().unwrap(),
        0,
        "no candidates left"
    );
}
...
async fn backfill_is_bounded_by_a_prior_deployment() {
    ...
    assert_eq!(
        e2e::poll_job(&app, &token, resp["job_id"].as_str().unwrap(), 30).await,
        "completed"
    );
    ...
    assert_eq!(
        by_time("2025-05-01"),
        Some(b.id),
        "gap reading attributed to B"
    );
    assert_eq!(
        by_time("2025-03-01"),
        Some(a.id),
        "in-A-window reading attributed to A"
    );
    assert_eq!(
        &deployed_from(&db, dep_b).await[..10],
        "2025-05-01",
