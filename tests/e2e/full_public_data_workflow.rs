...
async fn test_full_public_data_workflow() {
    ...
    let (_s, temp_param) = crate::common::post_json_parse_with_token(
        &app,
        "/api/parameters",
        &serde_json::json!({
    ...
    let (status, cal) = crate::common::post_json_with_token(
        &app,
    ...
    let (status, readings) = crate::common::get_json(&app, &readings_uri).await;
