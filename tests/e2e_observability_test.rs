...
async fn sensor_vs_grab_comparison_export() {
    let db = common::setup_test_db().await;
    common::cleanup_test_db(&db).await;
    common::seed_test_data(&db).await; // continuous readings at SITE1 for DO, 10-min cadence from 2025-01-15
    let token = common::seed_api_token(&db, common::full_permissions(), None).await;
    let app = common::build_test_app(db.clone());

    let site1 = common::SITE1_ID;
    let dop = common::GLOBAL_PARAM_DO_ID;
    ...
    let (status, readings) = common::get_json_with_token(&app, &rd, &token).await;
    ...
    let (status, spot) = common::get_json_with_token(&app, &spot_uri, &token).await;
    ...
    let (status, cmp) = common::get_json_with_token(&app, &exp, &token).await;
    ...
    let (status, ccsv) = common::get_csv_with_token(&app, &format!("{exp}&format=csv"), &token).await;
