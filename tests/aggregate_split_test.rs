...
//!
//! Run: cargo test --test aggregate_split_test -- --test-threads=1
...
async fn aggregates_split_by_sensor_returns_one_series_per_sensor() {
    ...
    let stream = sl::create_paired_stream(&db, "agg-split", common::PARAM_S1_TEMP_ID).await;
    sl::insert_readings(
    ...
    let (status, agg) = common::get_json_with_token(&app, &base, &token).await;
    ...
    let (status, agg) =
        common::get_json_with_token(&app, &format!("{base}&split_by_sensor=true"), &token).await;
