...
//!
//! Run with: cargo test --test alarm_events_test
...
async fn test_alarm_events_feed_and_filters() {
    ...
    let (status, body) = common::get_json_with_token(&app, "/api/alarms/events", &token).await;
    ...
    let (status, body) =
        common::get_json_with_token(&app, "/api/alarms/events?severity=2", &token).await;
    assert_eq!(status, 200);
    ...
    let (status, body) =
        common::get_json_with_token(&app, "/api/alarms/events?status=open", &token).await;
    assert_eq!(status, 200);
    assert_eq!(body["total"].as_u64(), Some(1));
    let events = body["events"].as_array().unwrap();
    assert_eq!(events.len(), 1);
    ...
    let (status, body) =
        common::get_json_with_token(&app, "/api/alarms/events?status=resolved", &token).await;
    assert_eq!(status, 200);
    assert_eq!(body["total"].as_u64(), Some(1));
    let events = body["events"].as_array().unwrap();
    ...
    let other_site = common::SITE2_ID;
    let (status, body) = common::get_json_with_token(
        &app,
...
async fn test_alarm_events_severity_filter_binds_to_max_severity() {
    ...
    let (status, body) =
        common::get_json_with_token(&app, "/api/alarms/events?severity=1", &token).await;
    assert_eq!(status, 200);
    assert_eq!(body["total"].as_u64(), Some(0));
...
async fn test_alarm_summary_includes_last_warning_and_alarm() {
    ...
    let (status, body) = common::get_json_with_token(&app, "/api/alarms/summary", &token).await;
    assert_eq!(status, 200);

    let by_site = body["by_site"].as_array().unwrap();
    let site1 = by_site
        .iter()
        .find(|s| s["site_id"].as_str() == Some(common::SITE1_ID))
