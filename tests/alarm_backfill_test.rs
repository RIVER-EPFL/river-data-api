...
//!
//! Run: cargo test --test alarm_backfill_test -- --test-threads=1
...
async fn csv_import_triggers_alarm_backfill_with_warnings_and_alarms() {
    ...
    let (status, events) = common::get_json_with_token(
        &app,
        &format!("/api/alarms/events?site_id={}", common::SITE1_ID),
        &token,
    )
    .await;
    ...
    let turb_events: Vec<&serde_json::Value> = events["events"]
        ...
        .unwrap()
        .iter()
        .filter(|e| e["parameter_id"].as_str() == Some(common::GLOBAL_PARAM_TURB_ID))
        .collect();
