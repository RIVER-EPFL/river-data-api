...
//!
//! Run: cargo test --test alarm_lifecycle_test -- --test-threads=1
...
async fn inject(db: &sea_orm::DatabaseConnection, stream_id: Uuid, time: &str, value: f64) {
    db.execute(Statement::from_string(
        ...
        format!(
            ...
            param = common::GLOBAL_PARAM_TURB_ID,
        ),
    ))
    .await
    .unwrap();
...
async fn reset_to_defaults_re_enables_alarms() {
    ...
    common::seed_test_data(&db).await; // global Turbidity threshold: alarm > 500
...
async fn unacknowledge_clears_acknowledgement() {
    ...
    let find_turb = |active: &serde_json::Value| -> Option<serde_json::Value> {
        active["alarms"]
            .as_array()?
            .iter()
            .find(|a| a["parameter_id"].as_str() == Some(common::GLOBAL_PARAM_TURB_ID))
            .cloned()
    };

    let (_s, active) = common::get_json_with_token(&app, "/api/alarms/active", &token).await;
    let event_id = find_turb(&active).unwrap()["event_id"]
        .as_str()
        .unwrap()
        .to_string();

    let (status, _b) = common::post_json_with_token(
        &app,
    ...
    let (status, _b) = common::delete_with_token(
        &app,
        &format!("/api/alarms/{event_id}/acknowledge"),
        &token,
    )
