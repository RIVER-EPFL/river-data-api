...
async fn test_pair_stream_backfills_readings() {
    ...
    common::seed_data_stream(&db, stream_id, "test-pair", "pair-key-1").await;

    let bt = common::base_time();
    for i in 0..10 {
...
async fn test_pair_already_paired_stream_fails() {
    ...
    common::seed_paired_stream(&db, stream_id, "test-pair", "pair-key-2", common::PARAM_S1_DO_ID).await;
...
async fn test_unpair_stream_clears_readings() {
    ...
    common::seed_paired_stream(&db, stream_id, "test-unpair", "unpair-key-1", common::PARAM_S1_COND_ID).await;
...
async fn test_unpair_unpaired_stream_fails() {
    ...
    common::seed_data_stream(&db, stream_id, "test-unpair", "unpair-key-2").await;
...
async fn test_stream_stats() {
    ...
    common::seed_data_stream(&db, stream_id, "test-stats", "stats-key-1").await;
...
async fn test_stream_stats_empty() {
    ...
    common::seed_data_stream(&db, stream_id, "test-stats", "stats-key-2").await;
