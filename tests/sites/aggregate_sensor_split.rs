...
async fn aggregates_split_by_sensor_returns_one_series_per_sensor() {
    ...
    let cal_a =
        sl::add_calibration(&db, sensor_a.id, 1.0, 0.0, sl::dt("2025-01-15T00:00:00Z")).await;
    ...
    let cal_b =
        sl::add_calibration(&db, sensor_b.id, 1.0, 0.0, sl::dt("2025-01-15T02:00:00Z")).await;
    ...
    sl::insert_readings(
        ...
        0.0,
        &[
            (sl::dt("2025-01-15T00:15:00Z"), 10.0),
            (sl::dt("2025-01-15T00:45:00Z"), 12.0),
        ],
    )
    .await;
    sl::insert_readings(
        ...
        0.0,
        &[
            (sl::dt("2025-01-15T02:15:00Z"), 20.0),
            (sl::dt("2025-01-15T02:45:00Z"), 22.0),
        ],
    )
    .await;
    ...
    assert_eq!(
        collapsed.len(),
        1,
        "collapsed returns a single Temperature series: {agg}"
    );
    assert!(
        collapsed[0]
            .get("sensor_id")
    ...
    let (status, agg) =
        crate::common::get_json_with_token(&app, &format!("{base}&split_by_sensor=true"), &token)
            .await;
    ...
    assert_eq!(
        sensor_ids, expected,
        "split returns one series per sensor with sensor_id: {agg}"
    );
}
