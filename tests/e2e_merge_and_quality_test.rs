...
//!
//! Run: cargo test --test e2e_merge_and_quality_test -- --test-threads=1
...
async fn merge_site_parameters_then_flag_and_query_alarms() {
    ...
    let (status, merge) = common::post_json_parse_with_token(
        &app,
        "/api/actions/merge_site_parameters",
        &serde_json::json!({
            "source_site_parameter_id": common::PARAM_S1_TEMP_ID,
            "target_site_parameter_id": common::PARAM_S1_DEPTH_ID,
        }),
        &token,
    )
    .await;
    ...
    let (status, _gone) =
        common::get_with_token(&app, &format!("/api/site_parameters/{}", common::PARAM_S1_TEMP_ID), &token).await;
    ...
    let key = serde_json::json!({ "site_id": site1, "parameter_id": common::GLOBAL_PARAM_COND_ID, "time": flag_time });
    ...
    let (status, summary) = common::get_json_with_token(&app, "/api/alarms/summary", &token).await;
