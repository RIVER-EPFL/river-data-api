...
//!
//! Run: cargo test --test e2e_deployment_twin_test -- --test-threads=1
...
async fn slot_exclusion_rejects_a_second_sensor() {
    ...
    let sensor_a = sl::create_sensor(&db, "slot-a", common::GLOBAL_PARAM_TEMP_ID).await;
    let sensor_b = sl::create_sensor(&db, "slot-b", common::GLOBAL_PARAM_TEMP_ID).await;

    // A takes the (site 1, Temperature) slot.
    let _dep_a = e2e::create_deployment(
        &app, &token, &sensor_a.id.to_string(), common::SITE1_ID, "2025-06-01T00:00:00Z",
    )
    .await;
...
async fn editing_deployment_start_rechains_previous() {
    ...
    let sensor = sl::create_sensor(&db, "mover", common::GLOBAL_PARAM_TEMP_ID).await;
    ...
    let dep1 = e2e::create_deployment(&app, &token, &sensor_id, common::SITE1_ID, "2025-06-01T00:00:00Z").await;
    let dep2 = e2e::create_deployment(&app, &token, &sensor_id, common::SITE2_ID, "2025-06-01T02:00:00Z").await;
    assert_eq!(
...
async fn recall_unattributes_post_recall_readings() {
    ...
    let sensor = sl::create_sensor(&db, "recall", common::GLOBAL_PARAM_TEMP_ID).await;
    ...
    let stream = sl::create_paired_stream(&db, "recall", common::PARAM_S1_TEMP_ID).await;
    let raw: Vec<(_, f64)> = (0..6)
        .map(|i| (sl::dt(&format!("2025-06-01T00:{:02}:00Z", i * 10)), 10.0 + i as f64))
        .collect();
    sl::insert_readings(
        &db, stream, common::SITE1_ID, common::GLOBAL_PARAM_TEMP_ID, sensor.id, cal, dep, 1.0, 0.0, &raw,
    )
    .await;
