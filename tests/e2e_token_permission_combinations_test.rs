...
async fn all_sixteen_permission_combinations_enforced() {
    ...
    for bits in 0u8..16 {
        ...
        let token = common::seed_api_token(
            &db,
            common::perms(read_metadata, read_data, write_metadata, write_data),
            None,
        )
        .await;
        ...
        let (s, _) = common::get_with_token(&app, "/api/annotations", &token).await;
        assert_eq!(s == 200, read_data, "[{label}] GET /api/annotations needs read_data: got {s}");
        let (s, _) = common::get_with_token(&app, "/api/samples", &token).await;
        ...
        let (s, _) = common::post_json_with_token(&app, "/api/parameters", &new_param, &token).await;
        if write_metadata {
        ...
        let (s, _) = common::post_json_with_token(&app, "/api/readings/batch", &batch, &token).await;
        if write_data {
