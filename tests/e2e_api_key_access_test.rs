...
use common::fixtures::{GLOBAL_PARAM_DEPTH_ID, GLOBAL_PARAM_TEMP_ID, PROJECT_ID, SITE1_ID};
use common::sensor_lifecycle as slc;
...
async fn setup() -> (DatabaseConnection, axum::Router, river_db::common::AppState) {
    ...
    .await;
    let (app, state) = common::build_test_app_with_state(db.clone());
    (db, app, state)
}
...
async fn auth_matrix_with_wrong_and_without_key() {
    ...
    let right = common::seed_api_token(&db, common::perms(true, true, false, true), Some(PROJECT_ID)).await;
    let (s, b) = common::post_json_with_token(&app, "/api/readings/batch", &body, &right).await;
    ...
    let (s, _) = common::get(&app, "/api/sites").await;
    assert_eq!(s, 401, "no key must be unauthorized");
    let (s, _) = common::post_json_with_token(&app, "/api/readings/batch", &body, "").await;
    ...
    let read_only = common::seed_api_token(&db, common::perms(true, true, false, false), Some(PROJECT_ID)).await;
    let (s, _) = common::post_json_with_token(&app, "/api/readings/batch", &body, &read_only).await;
    ...
    let other_project = common::seed_api_token(&db, common::perms(true, true, false, true), Some(PROJECT_B_ID)).await;
    let (s, _) = common::post_json_with_token(&app, "/api/readings/batch", &body, &other_project).await;
...
async fn write_paths_confined_ingest_status_and_flags() {
    ...
    let (s, b) = common::post_json_with_token(&app, "/api/ingest", &ingest_in, &key).await;
    ...
    let (s, _) = common::post_json_with_token(&app, "/api/ingest", &ingest_out, &key).await;
    ...
    let (s, _) = common::post_json_with_token(&app, "/api/ingest/status_events", &status_out, &key).await;
    ...
    let (s, b) = common::patch_json_with_token(&app, "/api/readings/flag", &flag_in, &key).await;
    ...
    let (s, _) = common::patch_json_with_token(&app, "/api/readings/flag", &flag_out, &key).await;
    ...
    let (s, _) = common::patch_json_with_token(&app, "/api/readings/flag_range", &range_out, &key).await;
    assert_eq!(s, 403, "cross-project flag_range must be forbidden");
    let (s, _) = common::patch_json_with_token(&app, "/api/readings/unflag_range", &range_out, &key).await;
...
async fn read_endpoints_filtered_to_token_project() {
    ...
    let key = common::seed_api_token(&db, common::perms(true, true, false, false), Some(PROJECT_ID)).await;
    ...
    let (s, j) = common::get_json_with_token(&app, "/api/search?q=Station", &key).await;
    assert_eq!(s, 200);
    ...
    let (_s, j) = common::get_json_with_token(&app, "/api/search?q=Station", &unscoped).await;
    ...
    let (s, _) = common::get_with_token(&app, &format!("/api/streams/{A_DEPTH_STREAM_ID}/stats"), &key).await;
    assert_eq!(s, 200, "scoped key reads its own stream stats");
    let (s, _) = common::get_with_token(&app, &format!("/api/streams/{b_stream}/stats"), &key).await;
    ...
    for path in [
        ...
        let (s, _) = common::get_with_token(&app, &path, &key).await;
    ...
    let (s, _) = common::get_with_token(&app, &format!("/api/sensor_calibrations/{cal_a}/window"), &key).await;
    assert_eq!(s, 200, "scoped key reads its own calibration window");
    let (s, _) = common::get_with_token(&app, &format!("/api/sensor_calibrations/{cal_b}/window"), &key).await;
    ...
    let (s, _) = common::get_with_token(&app, &format!("/api/sensors/{}/readings", sensor_b.id), &unscoped).await;
...
async fn api_token_use_is_recorded_in_audit_log() {
    ...
    common::cleanup_test_db(&db).await;
    common::seed_test_data(&db).await;
    let (app, state) = common::build_test_app_with_audit(db.clone());

    let key = common::seed_api_token(&db, common::full_permissions(), None).await;
    let tid = token_id(&db, &key).await;

    let (s, _) = common::get_with_token(&app, "/api/sites", &key).await;
    assert_eq!(s, 200);
