...
async fn issue(
    ...
    match (method, body) {
        ("GET", _) => match token {
            Some(t) => common::get_with_token(app, path, t).await.0,
            None => common::get(app, path).await.0,
        },
        ("POST", Some(b)) => match token {
            Some(t) => common::post_json_with_token(app, path, b, t).await.0,
            None => {
                let empty = "Bearer ";
                common::get_with_auth_header(app, path, empty).await.0
            }
        },
        ("POST", None) => match token {
            Some(t) => {
                common::post_json_with_token(app, path, &serde_json::json!({}), t).await.0
            }
            None => {
                let empty = "Bearer ";
                common::get_with_auth_header(app, path, empty).await.0
        ...
        ("PATCH", Some(b)) => match token {
            Some(t) => common::patch_json_with_token(app, path, b, t).await.0,
            None => 401,
        },
        ("DELETE", _) => match token {
            Some(t) => common::delete_with_token(app, path, t).await.0,
            None => 401,
        },
...
async fn permission_matrix_covers_every_boundary() {
    ...
    let site_id = common::fixtures::SITE1_ID;

    let tok_full = common::seed_token_full(&db).await;
    let tok_read_meta = common::seed_token_read_metadata_only(&db).await;
    let tok_read_data = common::seed_token_read_data_only(&db).await;
    let tok_write_meta = common::seed_token_write_metadata_only(&db).await;
    let tok_write_data = common::seed_token_write_data_only(&db).await;
    let (tok_sync, _service_id) = common::seed_sync_session_token(&db).await;
...
async fn require_admin_blocks_full_permissions_token() {
    ...
    for (method, path) in admin_paths {
        let status = match method {
            "GET" => common::get_with_token(&app, path, &tok_full).await.0,
            "POST" => {
                common::post_json_with_token(&app, path, &serde_json::json!({}), &tok_full)
                    .await
                    .0
            }
            _ => unreachable!(),
