...
async fn public_tier_exposes_only_public_flagged_data() {
    ...
    let (s, body) = crate::common::get_json(&app, &format!("/api/public/{PUBLIC_CODE}/sites")).await;
    assert_eq!(s, 200);
    ...
    let (s, _) = crate::common::get(&app, &format!("/api/public/{PUBLIC_CODE}/sites/upstream")).await;
    assert_eq!(s, 200, "public site is reachable");
    let (s, _) = crate::common::get(&app, &format!("/api/public/{PUBLIC_CODE}/sites/downstream")).await;
    assert_eq!(s, 404, "private site must be 404 on the public tier");

    // Parameters: only the public parameter is exposed.
    let (s, body) = crate::common::get_json(&app, &format!("/api/public/{PUBLIC_CODE}/sites/upstream/parameters")).await;
    assert_eq!(s, 200);
...
async fn private_project_and_coercion_are_blocked() {
    ...
    let (s, _) = crate::common::get(&app, &format!("/api/public/{PUBLIC_CODE}/sites/{SITE1_ID}")).await;
...
async fn public_tier_refuses_writes() {
    ...
    let (s, _) = crate::common::post_json(&app, &format!("/api/public/{PUBLIC_CODE}/sites"), &body).await;
