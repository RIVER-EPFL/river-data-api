...
async fn test_nonexistent_code_returns_404() {
    ...
    let (status, _body) = crate::common::get(&app, "/api/public/nonexistent-code/sites").await;
...
async fn test_non_public_project_returns_404() {
    ...
    let (status, _body) = crate::common::get(&app, "/api/public/test-river/sites").await;
...
async fn test_public_list_sites() {
    ...
    let (status, body) = crate::common::get_json(&app, "/api/public/test-river/sites").await;
...
async fn test_public_discovery() {
    ...
    let (status, body) = crate::common::get_json(&app, "/api/public").await;
    assert_eq!(status, 200);
