...
//! consistent.
//!
//! Run: cargo test --test pairing_plan_workflow_test -- --test-threads=1
...
async fn grouped_discovery_groups_unpaired_streams_by_site() {
    ...
    common::seed_unpaired_stream_with_hierarchy(
    ...
    let token = common::seed_token_full(&db).await;
    let app = common::build_test_app(db.clone());

    let (status, resp) = common::post_json_parse_with_token(
        &app,
...
async fn create_inspect_update_apply_revert_full_lifecycle() {
    ...
    common::seed_unpaired_stream_with_hierarchy(&db, &cond1, "nomis", "c1", "NOMIS", "GL1_DN", "Conductivity", "uS/cm", None, 3).await;
    common::seed_unpaired_stream_with_hierarchy(&db, &temp1, "nomis", "t1", "NOMIS", "GL1_DN", "Temperature", "degC", None, 3).await;
    common::seed_unpaired_stream_with_hierarchy(&db, &cond2, "nomis", "c2", "NOMIS", "GL2_UP", "Conductivity", "uS/cm", None, 3).await;
    common::seed_unpaired_stream_with_hierarchy(&db, &temp2, "nomis", "t2", "NOMIS", "GL2_UP", "Temperature", "degC", None, 3).await;
    let token = common::seed_token_full(&db).await;
    let app = common::build_test_app(db.clone());

    // create
    let (status, plan) = common::post_json_parse_with_token(
    ...
    // inspect
    let (status, plan) = common::get_json_with_token(&app, &format!("/api/sync/pairing-plans/{plan_id}"), &token).await;
    ...
    let (status, body) = common::patch_json_with_token(
        &app,
    ...
    // apply
    let (status, res) = common::post_json_parse_with_token(
    ...
    let (_, plan) = common::get_json_with_token(&app, &format!("/api/sync/pairing-plans/{plan_id}"), &token).await;
    ...
    // revert
    let (status, body) = common::post_json_parse_with_token(
...
async fn apply_reuses_existing_site_by_case_insensitive_name() {
    ...
    common::exec(&db, &format!("INSERT INTO projects (id, name) VALUES ('{project_id}', 'NOMIS')")).await;
    common::exec(&db, &format!(
    ...
    common::seed_unpaired_stream_with_hierarchy(&db, &stream, "nomis", "k1", "NOMIS", "GL1_DN", "Conductivity", "uS/cm", None, 0).await;
    let token = common::seed_token_full(&db).await;
    let app = common::build_test_app(db.clone());
...
async fn apply_backfills_coordinates_onto_existing_site_lacking_them() {
    ...
    let site_id = uuid::Uuid::new_v4().to_string();
    common::exec(&db, &format!("INSERT INTO sites (id, name) VALUES ('{site_id}', 'coordsite')")).await;
