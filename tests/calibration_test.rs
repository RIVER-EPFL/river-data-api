...
fn test_division_by_zero_produces_non_finite() {
    ...
    let result = river_db::services::calibration::evaluate_formula("a / b", &vars);
...
fn test_normal_formula_evaluation() {
    ...
    let result = river_db::services::calibration::evaluate_formula("a * b + 2", &vars);
    assert!(result.is_ok());
    let value = result.unwrap();
...
async fn test_derived_parameter_skips_infinity() {
    ...
    exec(
        &db,
        &format!(
            "INSERT INTO readings (site_id, parameter_id, time, raw_value) \
             VALUES ('{site_id}', '{do_param}', '{time}', 0.0) \
             ON CONFLICT (site_id, parameter_id, time) DO UPDATE SET raw_value = 0.0, calibrated_value = NULL",
    ...
    exec(
        ...
        &format!(
            "INSERT INTO readings (site_id, parameter_id, time, raw_value) \
             VALUES ('{site_id}', '{temp_param}', '{time}', 10.0) \
             ON CONFLICT (site_id, parameter_id, time) DO UPDATE SET raw_value = 10.0, calibrated_value = NULL",
    ...
    let result =
        river_db::services::calibration::recalculate_derived_at_timestamp(&db, site_uuid, time_dt)
            .await;
