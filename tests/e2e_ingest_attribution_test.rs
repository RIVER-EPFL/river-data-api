...
//!
//! Run: cargo test --test e2e_ingest_attribution_test -- --test-threads=1
...
async fn attr_at(
    ...
    let row = db
        .query_one(Statement::from_sql_and_values(
            ...
            [
                common::SITE1_ID.into(),
                common::GLOBAL_PARAM_TEMP_ID.into(),
                sl::dt(time).into(),
            ],
        ))
        .await
...
async fn batch_attributes_rows_inside_the_deployment_window() {
    ...
    let sensor = sl::create_sensor(&db, "ingest-probe", common::GLOBAL_PARAM_TEMP_ID).await;
    ...
    let (status, body) = common::post_json_with_token(
        ...
        &json!({
            "readings": [
                { "site_id": common::SITE1_ID, "parameter_id": common::GLOBAL_PARAM_TEMP_ID, "time": "2025-07-01T00:00:00Z", "raw_value": 21.0 },
                { "site_id": common::SITE1_ID, "parameter_id": common::GLOBAL_PARAM_TEMP_ID, "time": "2025-05-01T00:00:00Z", "raw_value": 19.0 }
            ]
        }),
        &token,
    )
...
async fn pairing_attributes_by_deployment_window() {
    ...
    let a = sl::create_sensor(&db, "incumbent-A", common::GLOBAL_PARAM_TEMP_ID).await;
    let dep_a = sl::deploy_sensor(&db, a.id, common::SITE1_ID, sl::dt("2025-06-01T00:00:00Z")).await;
    ...
    let (status, body) = common::post_json_with_token(
        ...
        &json!({ "site_parameter_id": common::PARAM_S1_TEMP_ID }),
        &token,
    )
    .await;
    ...
    for r in &rows {
        assert_eq!(r.site_id.map(|s| s.to_string()), Some(common::SITE1_ID.to_string()));
