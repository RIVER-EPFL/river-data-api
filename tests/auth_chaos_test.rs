...
async fn expired_token_returns_401_for_every_endpoint() {
    ...
    let tok = common::seed_api_token_with_expiry(
        &db,
...
async fn require_admin_blocks_every_bypass_vector() {
    ...
    let attempts: Vec<(&str, String)> = vec![
        (
            ...
            common::seed_api_token(&db, common::full_permissions(), None).await,
        ),
        (
            "project-scoped full token",
            common::seed_api_token(
                &db,
                common::full_permissions(),
        ...
        (
            ...
            common::seed_sync_session_token(&db).await.0,
        ),
    ];
    ...
    for (label, tok) in &attempts {
        for (method, path) in &admin_routes {
            let status = match *method {
                "GET" => common::get_with_token(&app, path, tok).await.0,
                "POST" => {
                    common::post_json_with_token(&app, path, &serde_json::json!({}), tok)
                        .await
                        .0
                }
                _ => unreachable!(),
