...
//!
//! Run with: cargo test --test samples_test
...
async fn samples_trigger_populates_aggregate_on_insert() {
    ...
    let stream_id = ensure_stream(&db, common::SITE1_ID, common::GLOBAL_PARAM_TEMP_ID).await;
    let sample_id =
        create_sample(&db, common::SITE1_ID, common::GLOBAL_PARAM_TEMP_ID, "test-happy").await;
...
async fn samples_trigger_recomputes_on_flag() {
    ...
    let stream_id = ensure_stream(&db, common::SITE1_ID, common::GLOBAL_PARAM_TEMP_ID).await;
    let sample_id =
        create_sample(&db, common::SITE1_ID, common::GLOBAL_PARAM_TEMP_ID, "test-flag").await;
...
async fn samples_trigger_handles_all_deleted() {
    ...
    let stream_id = ensure_stream(&db, common::SITE1_ID, common::GLOBAL_PARAM_TEMP_ID).await;
    let sample_id =
        create_sample(&db, common::SITE1_ID, common::GLOBAL_PARAM_TEMP_ID, "test-empty").await;
...
async fn samples_trigger_handles_reassignment() {
    ...
    let stream_id = ensure_stream(&db, common::SITE1_ID, common::GLOBAL_PARAM_TEMP_ID).await;
    let sample_a =
        create_sample(&db, common::SITE1_ID, common::GLOBAL_PARAM_TEMP_ID, "a").await;
...
async fn samples_delete_sets_reading_sample_id_null() {
    ...
    let stream_id = ensure_stream(&db, common::SITE1_ID, common::GLOBAL_PARAM_TEMP_ID).await;
    let sample_id =
        create_sample(&db, common::SITE1_ID, common::GLOBAL_PARAM_TEMP_ID, "del").await;
