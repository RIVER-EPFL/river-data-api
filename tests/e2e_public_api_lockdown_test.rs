...
use common::fixtures::{PARAM_S1_DO_ID, PARAM_S1_TEMP_ID, PROJECT_ID, SITE1_ID};
...
async fn setup() -> (sea_orm::DatabaseConnection, axum::Router) {
    ...
    for sql in [
        format!("UPDATE projects SET is_public = true, public_code = '{PUBLIC_CODE}' WHERE id = '{PROJECT_ID}'"),
...
async fn public_tier_exposes_only_public_flagged_data() {
    ...
    let (s, body) = common::get_json(&app, "/api/public").await;
    assert_eq!(s, 200, "discovery should be reachable");
    let codes: Vec<String> = body
        .as_array()
        .map(|a| a.iter().filter_map(|e| e["code"].as_str().map(str::to_string)).collect())
        .unwrap_or_default();
    assert!(codes.contains(&PUBLIC_CODE.to_string()), "public project must be discoverable: {body}");

    // Sites: the public site is listed, the private one is not.
    let (s, body) = common::get_json(&app, &format!("/api/public/{PUBLIC_CODE}/sites")).await;
    assert_eq!(s, 200);
    let body_str = body.to_string();
    assert!(body_str.contains("upstream"), "public site must be listed: {body}");
    assert!(!body_str.to_lowercase().contains("downstream"), "private site must NOT be listed: {body}");

    // The public site resolves; the private site 404s even by its real name.
    let (s, _) = common::get(&app, &format!("/api/public/{PUBLIC_CODE}/sites/upstream")).await;
    assert_eq!(s, 200, "public site is reachable");
    let (s, _) = common::get(&app, &format!("/api/public/{PUBLIC_CODE}/sites/downstream")).await;
    assert_eq!(s, 404, "private site must be 404 on the public tier");

    // Parameters: only the public parameter is exposed.
    let (s, body) = common::get_json(&app, &format!("/api/public/{PUBLIC_CODE}/sites/upstream/parameters")).await;
    assert_eq!(s, 200);
    let pbody = body.to_string();
    assert!(pbody.contains("DO_Temperature"), "public parameter must be exposed: {body}");
    assert!(!pbody.contains("Dissolved_O2"), "private parameter must NOT be exposed: {body}");
}
...
async fn private_project_and_coercion_are_blocked() {
    ...
    for code in ["nonexistent", "Secret"] {
        let (s, _) = common::get(&app, &format!("/api/public/{code}/sites")).await;
    ...
    let (s, _) = common::get(&app, &format!("/api/public/{PUBLIC_CODE}/sites/{SITE1_ID}")).await;
    assert_eq!(s, 404, "a site must be addressed by its public code, not its internal UUID");
}
...
async fn public_tier_refuses_writes() {
    ...
    let (s, _) = common::post_json(&app, &format!("/api/public/{PUBLIC_CODE}/sites"), &body).await;
    assert!(s == 405 || s == 404, "POST to a public route must be rejected, got {s}");
    let (s, _) = common::post_json(&app, "/api/public", &body).await;
    assert!(s == 405 || s == 404, "POST to public discovery must be rejected, got {s}");
}
