...
use common::fixtures::{GLOBAL_PARAM_TEMP_ID, PROJECT_ID, SITE1_ID};
...
async fn per_token_limit_recovers_after_refill() {
    ...
    let key = common::seed_api_token_with_rate_limit(
        &db,
    ...
    for _ in 0..10 {
        let (s, _) = common::post_json_with_token(&app, "/api/readings/batch", &reading_body(), &key).await;
        if s == 429 {
            hit_429 = true;
            break;
        }
...
async fn sync_session_token_is_never_throttled() {
    ...
    let (sync_token, _service_id) = common::seed_sync_session_token(&db).await;

    let mut statuses = Vec::new();
    for _ in 0..25 {
        let (s, _) = common::post_json_with_token(&app, "/api/readings/batch", &reading_body(), &sync_token).await;
        statuses.push(s);
    }
    assert!(
