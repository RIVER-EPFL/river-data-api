...
/// Scenario: a calculation finding is dismissed from the route another screen owns.
///
/// Expected behaviour: the route refuses it by kind, so no route can mint another purpose's
/// sentence over a hold it was never meant to reach.
#[tokio::test]
#[serial]
async fn a_route_refuses_a_kind_it_does_not_own() {
    let (db, app, token) = setup().await;
    let hold_id = insert_event_finding(&db).await;

    for path in ["release_brake", "accept_identity", "accept_correction"] {
        let (status, body) = crate::common::post_json_with_token(
            &app,
            &format!("/api/sync/replicate_audit_holds/{hold_id}/{path}"),
            &json!({}),
            &token,
        )
        .await;
    ...
    let status_now: String = db
        ...
        .try_get("", "status")
        .unwrap();
    assert_eq!(status_now, "pending", "a refused route decided nothing");
    let annotations = crate::common::e2e::count(
        &db,
        &format!("SELECT COUNT(*) FROM annotations WHERE audit_hold_id = '{hold_id}'"),
    )
    .await;
    assert_eq!(annotations, 0, "a refused route minted nothing");
}
...
async fn an_event_finding_can_be_acknowledged() {
    ...
    let (status, body) = crate::common::post_json_with_token(
        &app,
        &format!("/api/sync/replicate_audit_holds/{hold_id}/dismiss_finding"),
        &json!({}),
        &token,
    )
    .await;
