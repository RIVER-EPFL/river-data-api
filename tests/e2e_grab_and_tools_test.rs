...
//!
//! Run: cargo test --test e2e_grab_and_tools_test -- --test-threads=1
...
async fn grab_replicates_aggregate_then_tool_result_saved_to_station() {
    ...
    let param = common::GLOBAL_PARAM_TEMP_ID; // seeded as a site_parameter at site 1
    ...
    let (status, doc) = common::post_json_parse_with_token(
        &app,
    ...
    let (status, save) = common::post_json_with_token(
        &app,
        "/api/grab_samples",
        &serde_json::json!({
