...
/// A rolled-back value correction restores the arrival stamp it recorded.
///
/// `ingested_at` is the arrival stamp of the value a row currently serves, and the correction arm
/// re-stamps it when the value moves. The rollback arm did not, so an inverted correction left the
/// original value reading as though it had arrived when the correction did. It is now one of the
/// columns a correction records as `old`, and the rollback restores it with the rest.
