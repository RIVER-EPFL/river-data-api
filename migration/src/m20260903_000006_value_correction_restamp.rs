...
/// `readings.ingested_at` is the arrival stamp of the current value. A `value_correction`
/// decision that changes `raw_value` re-stamps it in the same projection; one that re-asserts
/// the stored value leaves it alone.
