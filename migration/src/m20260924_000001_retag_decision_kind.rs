...
/// The ledger accepts `retag`, the kind a `measurement_retag` run records for each reading it
/// reclassified.
...
const KINDS: &str = "'flag', 'unflag', 'withdraw', 'reassert', 'curve', 'calibration_pin', \
     ...
     'derived_computed', 'reprocess', 'rollback'";
...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        manager
            .get_connection()
            .execute_unprepared(&check(", 'retag'"))
            .await?;
        Ok(())
    }
