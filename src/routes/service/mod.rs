...
pub fn api_router(state: &AppState) -> (Router<()>, utoipa::openapi::OpenApi) {
    ...
    let operator_action_routes = Router::new()
        ...
        // Applying a group declares the slots a site's calculations read and write, so it carries
        // the same MANAGER gate the other slot-shaping actions do.
        .route(
        ...
        // against what the calculation reads and creates the outputs it lacks, so it carries the
        // same MANAGER gate.
        .route(
