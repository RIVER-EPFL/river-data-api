...
/// Expected behaviour: an event input names its param and the parameter it reads, and nothing
/// about how it is reached: every source is read at the instant computed (Q252).
#[test]
fn test_an_event_input_names_no_alignment() {
    let pco2 = formula(
        "pco2_corr",
        1,
        "co2 * alkalinity",
    ...
    let manifest = manifest_json("pCO2", None, &[pco2], &[]).expect("the set has an order");
    for input in manifest["event_inputs"].as_array().unwrap() {
        assert!(input.get("alignment").is_none(), "{input}");
    }
}
