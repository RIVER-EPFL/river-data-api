...
use super::script_model::{ToolScript, ToolScriptList};
use super::version_model::ToolScriptVersion;
use super::{activation_model, engine, script_model, script_operations, version_model};
...
pub async fn create_script(
    ...
    let name = script_operations::normalise_name(&payload.name)
        .map_err(|e| AppError::BadRequest(e.to_string()))?;
    if let Some(engine) = payload.engine.as_deref() {
        script_operations::check_engine(engine).map_err(|e| AppError::BadRequest(e.to_string()))?;
    }
    let model = script_model::ActiveModel {
        id: Set(Uuid::new_v4()),
        name: Set(name.clone()),
...
pub async fn update_script(
    ...
    let existing = script_model::Entity::find_by_id(id)
        .one(&state.db)
        .await?
        .ok_or_else(|| AppError::NotFound(format!("tool script {id} not found")))?;
    let mut model: script_model::ActiveModel = existing.into();
...
async fn load_script(state: &AppState, id: Uuid) -> AppResult<ToolScript> {
...
async fn load_version(state: &AppState, script_id: Uuid, vid: Uuid) -> AppResult<ToolScriptVersion> {
    version_model::Entity::find_by_id(vid)
        .filter(version_model::Column::ToolScriptId.eq(script_id))
        .one(&state.db)
        .await?
...
pub async fn list_activations(
    ...
    let rows = activation_model::Entity::find()
        .filter(activation_model::Column::ToolScriptId.eq(id))
        .order_by_desc(activation_model::Column::ActivatedAt)
        .all(&state.db)
        .await?;
    // The audit stores version ids; a reader wants the numbers, and one lookup answers every row.
    let numbers: HashMap<Uuid, i32> = version_model::Entity::find()
        .filter(version_model::Column::ToolScriptId.eq(id))
        .all(&state.db)
        .await?
        .into_iter()
