...
/// Expected behaviour: a window recompute reads the instants the written parameter is read at, and
/// nothing after them (Q252).
#[test]
fn test_a_window_recompute_is_the_written_instants() {
    ...
    assert!(
        !statements[0].sql.contains("alignment"),
        "{}",
        statements[0].sql
    );
}
